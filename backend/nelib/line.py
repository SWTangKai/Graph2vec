...
from nelib.classify import Classifier, read_node_label
...
class _LINE(object):
    ...
    def build_graph(self):
        ...
        self.context_embeddings = tf.get_variable(name="context_embeddings"+str(self.order), shape=[
                                                  self.node_size, self.rep_size], initializer=tf.contrib.layers.xavier_initializer(uniform=False, seed=cur_seed))
        self.pos_h_e = tf.nn.l2_normalize(
            tf.nn.embedding_lookup(self.embeddings, self.pos_h), 1)
        self.pos_t_e = tf.nn.l2_normalize(
            tf.nn.embedding_lookup(self.embeddings, self.pos_t), 1)
        self.pos_t_e_context = tf.nn.l2_normalize(
            tf.nn.embedding_lookup(self.context_embeddings, self.pos_t), 1)
        self.pos_h_v_e = tf.nn.l2_normalize(
            tf.nn.embedding_lookup(self.embeddings, self.pos_h_v), 2)
        self.neg_t_e = tf.nn.l2_normalize(
            tf.nn.embedding_lookup(self.embeddings, self.neg_t), 2)
        self.neg_t_e_context = tf.nn.l2_normalize(
            tf.nn.embedding_lookup(self.context_embeddings, self.neg_t), 2)
        ...
        #                            self.sample_sum1)
        self.sample_sum2 = tf.reduce_sum(tf.exp(tf.reduce_sum(
            tf.multiply(self.pos_h_v_e, self.neg_t_e_context), axis=2)), axis=1)
        ...
        self.sample_sum1 = tf.reduce_sum(tf.exp(tf.reduce_sum(
            tf.multiply(self.pos_h_v_e, self.neg_t_e), axis=2)), axis=1)
    ...
    def get_embeddings(self):
        vectors = {}
        embeddings = self.sess.run(tf.nn.l2_normalize(
            self.embeddings.eval(session=self.sess), 1))
...
class LINE(object):
    ...
    def __init__(self, graph, rep_size=128, batch_size=1000, epoch=10, negative_ratio=5, order=3, label_file=None, clf_ratio=0.5, auto_stop=True):
        ...
        self.vectors = {}
        if order == 3:
            self.model1 = _LINE(graph, rep_size/2,
                                batch_size, negative_ratio, order=1)
            self.model2 = _LINE(graph, rep_size/2,
                                batch_size, negative_ratio, order=2)
    ...
    def get_embeddings(self):
        ...
        if self.order == 3:
            ...
            for node in list(vectors1.keys()):
    ...
    def save_embeddings(self, filename):
        ...
        node_num = len(list(self.vectors.keys()))
