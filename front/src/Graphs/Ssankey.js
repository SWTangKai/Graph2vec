    ...
    render(data, ID) {
        ...
        data.links.forEach(function(d){
            if(+d.source == +ID && +d.target != +ID){
                fulllink.push({"source":"0-" + idtoc[d.source],"target":"1-" + idtoc[d.target]})
        ...
        linktemp.forEach(function(d){
            ...
            if(nodetemp.indexOf(d.source) > -1){
                if(nodetemp.indexOf(d.target) == -1){
                   ...
                   fulllink.push({ "source":"1-" + idtoc[d.source],
        ...
        let margin = {top: this.height * 0.1, bottom: this.height * 0.1, left: this.width * 0.1, right: this.width * 0.1}
        let width = this.width - margin.left - margin.right,
            height = this.height - margin.top - margin.bottom;

        let svg = d3.select(domName).append('svg')
